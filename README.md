...
- `[llm]`: configures the chosen `sonic182/llm-async` provider (provider id, API key, model, temperature/token flags, token limits, `max_tool_iterations`, system prompt, etc.). For `openai_responses` + models that reject reasoning params (for example `gpt-4.1-mini`), set `send_reasoning_effort = false`. For OpenRouter, optional `llm.openrouter.models` lets you provide a fallback model pool and `llm.openrouter.provider` lets you send routing controls (`order`, `allow_fallbacks`, `only`, `ignore`, `sort`, throughput/latency preferences, `max_price`, and `provider_extra` for future keys).
