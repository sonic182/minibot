...
- `[tools.http_client]`: toggles the HTTP client tool. Configure timeout, `max_bytes` (raw byte cap), optional `max_chars` (LLM-facing char cap), and `response_processing_mode` (`auto`/`none`) for response shaping via [aiosonic]. Enable `spill_to_managed_file` with `spill_after_chars`, `spill_preview_chars`, and `spill_subdir` to store oversized responses as managed temp files. When a response spills, the tool returns a preview in `body`, an explicit `body_notice`, and `body_file_path`/`body_file_absolute_path` so file and grep tools can inspect the full response.
