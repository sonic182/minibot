...
- **HTTP Tooling**: optional HTTP client tools use `aiosonic` to fetch external URLs with strict method/timeout/output caps and optional content-type-aware response processing (`response_processing_mode`, `max_chars`). Config toggles (`[tools.http_client]`) control availability per deployment.
